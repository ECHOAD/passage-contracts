...
use cosmwasm_std::{
    coin, to_json_binary, Addr, BankMsg, Binary, Coin, CosmosMsg, Deps, DepsMut, Env, Event,
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::NumMinted {} => to_json_binary(&query_num_minted(deps)?),
        QueryMsg::NumRemaining {} => to_json_binary(&query_num_remaining(deps)?),
        ...
        QueryMsg::TokenMint { token_id } => to_json_binary(&query_token_mint(deps, token_id)?),
        QueryMsg::TokenMints {
            descending,
            filter_minted,
            start_after,
            limit,
        } => to_json_binary(&query_token_mints(
            deps,
            descending,
            filter_minted,
            start_after,
