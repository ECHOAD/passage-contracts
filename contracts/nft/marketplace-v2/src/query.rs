...
use crate::msg::{
    AskCountResponse, AskResponse, AsksResponse, BidResponse, BidTokenPriceOffset, BidsResponse,
    CollectionBidPriceOffset, CollectionBidResponse, CollectionBidsResponse, ConfigResponse,
    QueryMsg, QueryOptions, TokenAddrOffset, TokenPriceOffset,
};
use crate::state::{asks, bid_key, bids, collection_bids, TokenId, CONFIG};
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    ...
    match msg {
        ...
        QueryMsg::AsksSortedByPrice { query_options } => {
            to_json_binary(&query_asks_sorted_by_price(deps, &query_options)?)
        }
        QueryMsg::AsksBySeller { query_options } => {
            to_json_binary(&query_asks_by_seller(deps, &query_options)?)
        }
        ...
        QueryMsg::BidsByTokenPrice {
            token_id,
            query_options,
        } => to_json_binary(&query_bids_token_price(deps, token_id, &query_options)?),
        QueryMsg::BidsByBidder { query_options } => {
            to_json_binary(&query_bids_by_bidder(deps, &query_options)?)
        }
        QueryMsg::CollectionBid { bidder } => {
            to_json_binary(&query_collection_bid(deps, api.addr_validate(&bidder)?)?)
        }
        QueryMsg::CollectionBidsByPrice { query_options } => {
            to_json_binary(&query_collection_bids_by_price(deps, &query_options)?)
        }
    }
}
...
pub fn query_asks_sorted_by_price(
    ...
    let start = query_options
        .start_after
        .as_ref()
        .map(|offset| Bound::exclusive((offset.price.u128(), offset.token_id.clone())));
...
pub fn query_asks_by_seller(
    ...
    let start = query_options
        .start_after
        .as_ref()
        .map(|offset| Bound::exclusive((offset.address.clone(), offset.token_id.clone())));
    let order = option_bool_to_order(query_options.descending);
...
pub fn query_bids_token_price(
    ...
    let start = query_options.start_after.as_ref().map(|offset| {
        Bound::exclusive((
            offset.price,
            bid_key(&offset.bidder, offset.token_id.clone()),
        ))
    });
    let order = option_bool_to_order(query_options.descending);
...
pub fn query_bids_by_bidder(
    ...
    let start = query_options
        .start_after
        .as_ref()
        .map(|offset| Bound::exclusive(bid_key(&offset.address, offset.token_id.clone())));
    let order = option_bool_to_order(query_options.descending);
...
pub fn query_collection_bid(deps: Deps, bidder: Addr) -> StdResult<CollectionBidResponse> {
...
pub fn query_collection_bids_by_price(
    ...
    let start = query_options
        .start_after
        .as_ref()
        .map(|offset| Bound::exclusive((offset.price, offset.bidder.clone())));
    let order = option_bool_to_order(query_options.descending);
