...
use crate::msg::{
    AuctionResponse, AuctionsResponse, ConfigResponse, QueryMsg, QueryOptions, TokenPriceOffset,
    TokenTimestampOffset,
};
use crate::state::{auctions, AuctionStatus, TokenId, CONFIG};
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    ...
    match msg {
        ...
        QueryMsg::Auction { token_id } => to_json_binary(&query_auction(deps, env, token_id)?),
        QueryMsg::AuctionsByStartTime { query_options } => {
            to_json_binary(&query_auctions_by_start_time(deps, &query_options)?)
        }
        QueryMsg::AuctionsByEndTime { query_options } => {
            to_json_binary(&query_auctions_by_end_time(deps, &query_options)?)
        }
        QueryMsg::AuctionsByHighestBidPrice { query_options } => {
            to_json_binary(&query_auctions_by_highest_bid_price(deps, &query_options)?)
        }
        QueryMsg::AuctionsBySellerEndTime {
            seller,
            query_options,
        } => to_json_binary(&query_auctions_by_seller_end_time(
            deps,
        ...
        QueryMsg::AuctionsByBidderEndTime {
            bidder,
            query_options,
        } => to_json_binary(&query_auctions_by_highest_bidder_end_time(
            deps,
...
pub fn query_auctions_by_start_time(
    ...
    let start = query_options
        .start_after
        .as_ref()
        .map(|offset| Bound::exclusive((offset.timestamp.seconds(), offset.token_id.clone())));
