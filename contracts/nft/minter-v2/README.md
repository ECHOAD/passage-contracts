...
2. Query `Config {}` or `MintPrice {}` and confirm `current_price.denom` / `unit_price.denom` is `upasg` when the drop is PASG-native.
3. Use `CanMint { address }` before sending funds.
4. After `Mint`, `BatchMint`, `Withdraw`, or `WithdrawTo`, inspect the execute response attributes:
   - `pasg_utility_query`
   - `pasg_native_denom`
