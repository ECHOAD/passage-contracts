...
use crate::state::{
    collections, CollectionCreationPolicy, Config, Ecosystem, EcosystemType, NftType, CONFIG,
    ECOSYSTEMS,
};
...
#[test]
fn deregistered_collections_become_unaffiliated() {
    let mut deps = mock_dependencies();
    let env = mock_env();
    let api = MockApi::default();
    let registry_admin = api.addr_make("registry_admin");
    ...
    execute(
        ...
        ExecuteMsg::DeregisterCollection {
            address: component_collection.to_string(),
        },
    )
    .unwrap();
    ...
    let stored = collections()
        .load(deps.as_ref().storage, component_collection.clone())
        .unwrap();
    assert_eq!(stored.ecosystem_id, None);

    let response: CollectionsResponse = from_json(
        query(
            deps.as_ref(),
            env,
            QueryMsg::UnaffiliatedCollections {
                start_after: None,
                limit: None,
            },
        )
    ...
    assert_eq!(response.collections.len(), 1);
    assert_eq!(response.collections[0].address, component_collection);
    assert_eq!(response.collections[0].ecosystem_id, None);
}
