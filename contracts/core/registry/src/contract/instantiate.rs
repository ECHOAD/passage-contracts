...
pub fn instantiate(
    ...
    RECOVERY_CASE_COUNT.save(deps.storage, &0u64)?;
    RECOVERY_CONFIG.save(
        deps.storage,
        &RecoveryConfig {
            abandonment_inactivity_period_secs: 90 * 24 * 60 * 60,
    ...
    for recovery_member in config.recovery_council {
        LAST_CREATOR_ACTIVITY.save(deps.storage, recovery_member, &now)?;
    }
